...
export function previewBasket(input: BasketInput, metas: PerpMetaMap): BasketPreview {
  ...
  for (const s of symbols) {
    ...
    const px = m?.markPx;
    if (!px || px <= 0) { 
      skipped.push({ symbol: s, reason: 'no price' }); 
      continue; 
    }
    
    const minUsd = Math.max(m?.minOrderUsd ?? 0, SAFE_MIN_USD);
    if (perSymbolUsd < minUsd) { 
      skipped.push({ symbol: s, reason: `below min ${minUsd}` }); 
      continue; 
    }
    ...
    if (sz <= 0) { 
      skipped.push({ symbol: s, reason: 'size=0' }); 
      continue; 
    }
    ...
    if (orderType === 'limit') {
      ...
      if (!lpx || lpx <= 0) { 
        skipped.push({ symbol: s, reason: 'no limit px' }); 
        continue; 
      }
      o.px = lpx;
    }
